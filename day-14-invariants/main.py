...
def print_task(agent: BublikAgent) -> None:
    ...
    print(f"Этап: {task.state.value}")
    print(f"Пауза: {'да' if task.paused else 'нет'}")
    print(f"Прогресс: {len(task.done)}/{task.total}")
    print(f"Текущий шаг: {task.current or '—'}")
    print(f"Ожидаемое действие: {task.expected_action.value}")
...
def main() -> None:
    ...
    print("🤖 День 14 — инварианты и ограничения состояния")
    while True:
