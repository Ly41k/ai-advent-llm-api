"""Подсчёт токенов и стоимости запросов агента."""
...
class RequestTokenUsage:
    """Фактические API-метрики одного завершённого запроса."""
...
class GptOssTokenCounter:
    ...
    def count_messages(
        ...
        if add_generation_prompt:
            rendered_prompt += "<|start|>assistant"
        return len(
    ...
    def build_usage(
        ...
        """Объединяет локальные оценки с фактическими данными Groq."""

        cost = (
            api_prompt_tokens * INPUT_PRICE_PER_MILLION
...
def estimate_request_cost(prompt_tokens: int, completion_tokens: int) -> float:
    """Оценивает стоимость запроса по текущим тарифам GPT-OSS 20B."""
