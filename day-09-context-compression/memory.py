"""SQLite-хранилище полной истории, summary и токенов."""

import sqlite3
from dataclasses import dataclass
from datetime import datetime, timezone
from pathlib import Path
from uuid import uuid4

from tokens import RequestTokenUsage, SummaryTokenUsage


@dataclass(frozen=True)
class Conversation:
    """Независимый диалог, доступный после перезапуска."""

    id: str
    title: str
    message_count: int
    updated_at: str


@dataclass(frozen=True)
class Message:
    """Сообщение из полной истории диалога."""

    id: int
    role: str
    content: str
    status: str
    created_at: str


@dataclass(frozen=True)
class ConversationSummary:
    """Отдельно сохранённое сжатие старой части диалога."""

    content: str
    summarized_through_message_id: int
    source_message_count: int
    update_count: int
    prompt_tokens: int
    completion_tokens: int
    total_tokens: int
    estimated_cost_usd: float
    updated_at: str


@dataclass(frozen=True)
class ConversationUsage:
    """Накопленная стоимость обычных запросов выбранного диалога."""

    request_count: int
    prompt_tokens: int
    completion_tokens: int
    total_tokens: int
    estimated_cost_usd: float


class SQLiteContextRepository:
    """Хранит полную историю и независимый накопительный summary."""

    def __init__(self, database_path: Path) -> None:
        self._database_path = database_path
        self._database_path.parent.mkdir(parents=True, exist_ok=True)
        self._create_tables()

    def _connect(self) -> sqlite3.Connection:
        connection = sqlite3.connect(self._database_path)
        connection.row_factory = sqlite3.Row
        connection.execute("PRAGMA foreign_keys = ON")
        return connection

    def _create_tables(self) -> None:
        with self._connect() as connection:
            connection.executescript(
                """
                CREATE TABLE IF NOT EXISTS conversations (
                    id TEXT PRIMARY KEY,
                    title TEXT NOT NULL,
                    created_at TEXT NOT NULL,
                    updated_at TEXT NOT NULL
                );

                CREATE TABLE IF NOT EXISTS messages (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    conversation_id TEXT NOT NULL,
                    role TEXT NOT NULL CHECK (role IN ('user', 'assistant')),
                    content TEXT NOT NULL,
                    status TEXT NOT NULL CHECK (
                        status IN ('pending', 'completed', 'failed')
                    ),
                    error_message TEXT,
                    created_at TEXT NOT NULL,
                    FOREIGN KEY (conversation_id)
                        REFERENCES conversations(id) ON DELETE CASCADE
                );

                CREATE INDEX IF NOT EXISTS index_messages_conversation
                ON messages(conversation_id, id);

                CREATE TABLE IF NOT EXISTS conversation_summaries (
                    conversation_id TEXT PRIMARY KEY,
                    content TEXT NOT NULL,
                    summarized_through_message_id INTEGER NOT NULL,
                    source_message_count INTEGER NOT NULL,
                    update_count INTEGER NOT NULL,
                    prompt_tokens INTEGER NOT NULL,
                    completion_tokens INTEGER NOT NULL,
                    total_tokens INTEGER NOT NULL,
                    estimated_cost_usd REAL NOT NULL,
                    updated_at TEXT NOT NULL,
                    FOREIGN KEY (conversation_id)
                        REFERENCES conversations(id) ON DELETE CASCADE,
                    FOREIGN KEY (summarized_through_message_id)
                        REFERENCES messages(id) ON DELETE CASCADE
                );

                CREATE TABLE IF NOT EXISTS request_usage (
                    user_message_id INTEGER PRIMARY KEY,
                    conversation_id TEXT NOT NULL,
                    current_request_tokens INTEGER NOT NULL,
                    full_history_tokens INTEGER NOT NULL,
                    compressed_history_tokens INTEGER NOT NULL,
                    saved_history_tokens INTEGER NOT NULL,
                    estimated_prompt_tokens INTEGER NOT NULL,
                    api_prompt_tokens INTEGER NOT NULL,
                    completion_tokens INTEGER NOT NULL,
                    visible_response_tokens INTEGER NOT NULL,
                    total_tokens INTEGER NOT NULL,
                    estimated_cost_usd REAL NOT NULL,
                    FOREIGN KEY (user_message_id)
                        REFERENCES messages(id) ON DELETE CASCADE,
                    FOREIGN KEY (conversation_id)
                        REFERENCES conversations(id) ON DELETE CASCADE
                );

                CREATE INDEX IF NOT EXISTS index_usage_conversation
                ON request_usage(conversation_id, user_message_id);
                """
            )

    def create_conversation(self, title: str) -> Conversation:
        """Создаёт новую независимую тему."""

        prepared_title = title.strip()
        if not prepared_title:
            raise ValueError("Название диалога не может быть пустым")

        conversation_id = str(uuid4())
        created_at = self._now()
        with self._connect() as connection:
            connection.execute(
                """
                INSERT INTO conversations(id, title, created_at, updated_at)
                VALUES (?, ?, ?, ?)
                """,
                (conversation_id, prepared_title, created_at, created_at),
            )
        return Conversation(
            id=conversation_id,
            title=prepared_title,
            message_count=0,
            updated_at=created_at,
        )

    def get_conversations(self) -> list[Conversation]:
        """Возвращает диалоги, начиная с самого свежего."""

        with self._connect() as connection:
            rows = connection.execute(
                """
                SELECT
                    conversations.id,
                    conversations.title,
                    conversations.updated_at,
                    COUNT(
                        CASE WHEN messages.status = 'completed' THEN 1 END
                    ) AS message_count
                FROM conversations
                LEFT JOIN messages
                    ON messages.conversation_id = conversations.id
                GROUP BY conversations.id
                ORDER BY conversations.updated_at DESC
                """
            ).fetchall()
        return [
            Conversation(
                id=row["id"],
                title=row["title"],
                message_count=int(row["message_count"]),
                updated_at=row["updated_at"],
            )
            for row in rows
        ]

    def conversation_exists(self, conversation_id: str) -> bool:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT 1 FROM conversations WHERE id = ?",
                (conversation_id,),
            ).fetchone()
        return row is not None

    def start_user_request(self, conversation_id: str, content: str) -> int:
        """Сохраняет запрос до обращения к основной модели."""

        with self._connect() as connection:
            cursor = connection.execute(
                """
                INSERT INTO messages(
                    conversation_id, role, content, status, created_at
                )
                VALUES (?, 'user', ?, 'pending', ?)
                """,
                (conversation_id, content, self._now()),
            )
            return int(cursor.lastrowid)

    def complete_exchange(
        self,
        user_message_id: int,
        conversation_id: str,
        assistant_content: str,
        usage: RequestTokenUsage,
    ) -> None:
        """Атомарно сохраняет ответ и метрики основного запроса."""

        with self._connect() as connection:
            cursor = connection.execute(
                """
                UPDATE messages
                SET status = 'completed'
                WHERE id = ? AND conversation_id = ? AND status = 'pending'
                """,
                (user_message_id, conversation_id),
            )
            if cursor.rowcount != 1:
                raise RuntimeError("Не найден ожидающий пользовательский запрос")

            connection.execute(
                """
                INSERT INTO messages(
                    conversation_id, role, content, status, created_at
                )
                VALUES (?, 'assistant', ?, 'completed', ?)
                """,
                (conversation_id, assistant_content, self._now()),
            )
            connection.execute(
                """
                INSERT INTO request_usage(
                    user_message_id,
                    conversation_id,
                    current_request_tokens,
                    full_history_tokens,
                    compressed_history_tokens,
                    saved_history_tokens,
                    estimated_prompt_tokens,
                    api_prompt_tokens,
                    completion_tokens,
                    visible_response_tokens,
                    total_tokens,
                    estimated_cost_usd
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    user_message_id,
                    conversation_id,
                    usage.current_request_tokens,
                    usage.full_history_tokens,
                    usage.compressed_history_tokens,
                    usage.saved_history_tokens,
                    usage.estimated_prompt_tokens,
                    usage.api_prompt_tokens,
                    usage.completion_tokens,
                    usage.visible_response_tokens,
                    usage.total_tokens,
                    usage.estimated_cost_usd,
                ),
            )
            connection.execute(
                "UPDATE conversations SET updated_at = ? WHERE id = ?",
                (self._now(), conversation_id),
            )

    def fail_request(self, user_message_id: int, error_message: str) -> None:
        """Сохраняет неуспешный запрос для диагностики."""

        with self._connect() as connection:
            connection.execute(
                """
                UPDATE messages
                SET status = 'failed', error_message = ?
                WHERE id = ? AND status = 'pending'
                """,
                (error_message, user_message_id),
            )

    def get_context_messages(self, conversation_id: str) -> list[Message]:
        """Возвращает полную успешную историю без удаления сообщений."""

        with self._connect() as connection:
            rows = connection.execute(
                """
                SELECT id, role, content, status, created_at
                FROM messages
                WHERE conversation_id = ? AND status = 'completed'
                ORDER BY id ASC
                """,
                (conversation_id,),
            ).fetchall()
        return [self._to_message(row) for row in rows]

    def get_unsummarized_context_messages(
        self,
        conversation_id: str,
    ) -> list[Message]:
        """Возвращает все сообщения после сохранённой позиции summary."""

        summary = self.get_summary(conversation_id)
        summarized_through = (
            summary.summarized_through_message_id if summary else 0
        )
        with self._connect() as connection:
            rows = connection.execute(
                """
                SELECT id, role, content, status, created_at
                FROM messages
                WHERE conversation_id = ?
                  AND status = 'completed'
                  AND id > ?
                ORDER BY id ASC
                """,
                (conversation_id, summarized_through),
            ).fetchall()
        return [self._to_message(row) for row in rows]

    def get_messages_for_summary(
        self,
        conversation_id: str,
        recent_message_limit: int,
        batch_size: int,
    ) -> list[Message]:
        """Выбирает очередную полную партию старых несжатых сообщений."""

        summary = self.get_summary(conversation_id)
        summarized_through = (
            summary.summarized_through_message_id if summary else 0
        )
        with self._connect() as connection:
            rows = connection.execute(
                """
                SELECT id, role, content, status, created_at
                FROM messages
                WHERE conversation_id = ?
                  AND status = 'completed'
                  AND id > ?
                ORDER BY id ASC
                """,
                (conversation_id, summarized_through),
            ).fetchall()

        messages = [self._to_message(row) for row in rows]
        eligible_count = len(messages) - recent_message_limit
        if eligible_count < batch_size:
            return []
        return messages[:batch_size]

    def get_summary(self, conversation_id: str) -> ConversationSummary | None:
        """Возвращает отдельно сохранённый summary выбранного диалога."""

        with self._connect() as connection:
            row = connection.execute(
                """
                SELECT
                    content,
                    summarized_through_message_id,
                    source_message_count,
                    update_count,
                    prompt_tokens,
                    completion_tokens,
                    total_tokens,
                    estimated_cost_usd,
                    updated_at
                FROM conversation_summaries
                WHERE conversation_id = ?
                """,
                (conversation_id,),
            ).fetchone()
        if row is None:
            return None
        return ConversationSummary(
            content=row["content"],
            summarized_through_message_id=int(
                row["summarized_through_message_id"]
            ),
            source_message_count=int(row["source_message_count"]),
            update_count=int(row["update_count"]),
            prompt_tokens=int(row["prompt_tokens"]),
            completion_tokens=int(row["completion_tokens"]),
            total_tokens=int(row["total_tokens"]),
            estimated_cost_usd=float(row["estimated_cost_usd"]),
            updated_at=row["updated_at"],
        )

    def save_summary(
        self,
        conversation_id: str,
        content: str,
        source_messages: list[Message],
        usage: SummaryTokenUsage,
    ) -> None:
        """Атомарно заменяет summary и накапливает стоимость его обновлений."""

        if not source_messages:
            raise ValueError("Нельзя сохранить summary без исходных сообщений")

        summarized_through = source_messages[-1].id
        updated_at = self._now()
        with self._connect() as connection:
            connection.execute(
                """
                INSERT INTO conversation_summaries(
                    conversation_id,
                    content,
                    summarized_through_message_id,
                    source_message_count,
                    update_count,
                    prompt_tokens,
                    completion_tokens,
                    total_tokens,
                    estimated_cost_usd,
                    updated_at
                )
                VALUES (?, ?, ?, ?, 1, ?, ?, ?, ?, ?)
                ON CONFLICT(conversation_id) DO UPDATE SET
                    content = excluded.content,
                    summarized_through_message_id =
                        excluded.summarized_through_message_id,
                    source_message_count =
                        conversation_summaries.source_message_count
                        + excluded.source_message_count,
                    update_count = conversation_summaries.update_count + 1,
                    prompt_tokens = conversation_summaries.prompt_tokens
                        + excluded.prompt_tokens,
                    completion_tokens = conversation_summaries.completion_tokens
                        + excluded.completion_tokens,
                    total_tokens = conversation_summaries.total_tokens
                        + excluded.total_tokens,
                    estimated_cost_usd =
                        conversation_summaries.estimated_cost_usd
                        + excluded.estimated_cost_usd,
                    updated_at = excluded.updated_at
                """,
                (
                    conversation_id,
                    content,
                    summarized_through,
                    len(source_messages),
                    usage.prompt_tokens,
                    usage.completion_tokens,
                    usage.total_tokens,
                    usage.estimated_cost_usd,
                    updated_at,
                ),
            )

    def get_history(
        self,
        conversation_id: str,
        limit: int = 20,
    ) -> list[Message]:
        """Возвращает журнал, включая неуспешные запросы."""

        with self._connect() as connection:
            rows = connection.execute(
                """
                SELECT id, role, content, status, created_at
                FROM (
                    SELECT id, role, content, status, created_at
                    FROM messages
                    WHERE conversation_id = ?
                    ORDER BY id DESC
                    LIMIT ?
                )
                ORDER BY id ASC
                """,
                (conversation_id, limit),
            ).fetchall()
        return [self._to_message(row) for row in rows]

    def get_completed_message_count(self, conversation_id: str) -> int:
        with self._connect() as connection:
            row = connection.execute(
                """
                SELECT COUNT(*) AS message_count
                FROM messages
                WHERE conversation_id = ? AND status = 'completed'
                """,
                (conversation_id,),
            ).fetchone()
        return int(row["message_count"])

    def get_conversation_usage(self, conversation_id: str) -> ConversationUsage:
        """Суммирует фактические токены и стоимость основных запросов."""

        with self._connect() as connection:
            row = connection.execute(
                """
                SELECT
                    COUNT(*) AS request_count,
                    COALESCE(SUM(api_prompt_tokens), 0) AS prompt_tokens,
                    COALESCE(SUM(completion_tokens), 0) AS completion_tokens,
                    COALESCE(SUM(total_tokens), 0) AS total_tokens,
                    COALESCE(SUM(estimated_cost_usd), 0.0) AS cost
                FROM request_usage
                WHERE conversation_id = ?
                """,
                (conversation_id,),
            ).fetchone()
        return ConversationUsage(
            request_count=int(row["request_count"]),
            prompt_tokens=int(row["prompt_tokens"]),
            completion_tokens=int(row["completion_tokens"]),
            total_tokens=int(row["total_tokens"]),
            estimated_cost_usd=float(row["cost"]),
        )

    @staticmethod
    def _to_message(row: sqlite3.Row) -> Message:
        return Message(
            id=int(row["id"]),
            role=row["role"],
            content=row["content"],
            status=row["status"],
            created_at=row["created_at"],
        )

    @staticmethod
    def _now() -> str:
        return datetime.now(timezone.utc).isoformat()
