"""Подсчёт токенов и стоимости разных стратегий контекста."""
...
class ContextTokenEstimate:
    ...
    def fits_context_window(self) -> bool:
        return self.total_reserved_tokens <= self.context_window
...
class GptOssTokenCounter:
    ...
    def count_messages(
        ...
        rendered = "".join(
            f"<|start|>{item['role']}<|message|>{item['content']}<|end|>\n"
            for item in messages
        )
        if add_generation_prompt:
            rendered += "<|start|>assistant"
        return len(self._encoding.encode(rendered, allowed_special="all"))
    ...
    def build_usage(
        ...
        return RequestTokenUsage(
            ...
            strategy_context_tokens=estimate.history_tokens,
            saved_history_tokens=max(full_history_tokens - estimate.history_tokens, 0),
            estimated_prompt_tokens=estimate.prompt_tokens,
            api_prompt_tokens=api_prompt_tokens,
            completion_tokens=completion_tokens,
            visible_response_tokens=self.count_text(response_text),
            total_tokens=total_tokens,
            estimated_cost_usd=estimate_request_cost(api_prompt_tokens, completion_tokens),
        )
