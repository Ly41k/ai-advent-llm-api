...
class SQLiteMemoryRepository:
    ...
    def _create_tables(self) -> None:
        with self._connect() as connection:
            connection.executescript(
                ...
                -- LONG-TERM: общие решения и знания доступны всем диалогам.
    ...
    def create_conversation(self, title: str) -> Conversation:
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                (conversation_id, prepared, now, now),
            )
        return Conversation(conversation_id, prepared, 0, now)
    ...
    def get_conversations(self) -> list[Conversation]:
        ...
        return [
            Conversation(row["id"], row["title"], row["message_count"], row["updated_at"])
            for row in rows
        ]
    ...
    def get_conversation(self, conversation_id: str) -> Conversation | None:
        ...
        if row is None:
            return None
        return Conversation(row["id"], row["title"], row["message_count"], row["updated_at"])
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                INSERT INTO working_memory(
                    ...
                    current, validation_passed, validation_details, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    # LONG-TERM MEMORY
    def save_long_term(self, kind: LongTermKind, key: str, value: str) -> None:
        prepared_key, prepared_value = self._prepare_pair(key, value)
        with self._connect() as connection:
            connection.execute(
                """INSERT INTO long_term_memory VALUES (?, ?, ?, ?)
                   ON CONFLICT(kind, key) DO UPDATE SET
                       value = excluded.value, updated_at = excluded.updated_at""",
                (kind.value, prepared_key, prepared_value, self._now()),
            )
