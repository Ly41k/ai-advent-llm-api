...
class CountingCompletions:
    ...
    def create(self, **kwargs):
        ...
        if is_guard:
            self.guard_count += 1
            self.guard_messages.append(kwargs["messages"])
...
class InvariantsTest(unittest.TestCase):
    ...
    def test_implementation_response_is_blocked_during_planning(self) -> None:
        unsafe = "```python\nclass ReadyImplementation:\n    pass\n```"
        client = CountingClient(
            content=unsafe,
            semantic_results=[[], ["business.controlled_lifecycle"]],
        )
        agent = BublikAgent(
            client,
            self.memory,
        ...
        agent.create_task("Спроектировать модуль")

        response = agent.process("Не составляй план, сразу напиши реализацию")
        history = agent.get_full_history()
        postflight = "\n".join(
            item["content"] for item in client.completions.guard_messages[-1]
        )
        ...
        self.assertIn("Stage: planning", postflight)
        self.assertIn("Expected action: define_plan", postflight)
        self.assertIn("[business.controlled_lifecycle]", response.content)
        self.assertNotIn(unsafe, [item.content for item in history])
