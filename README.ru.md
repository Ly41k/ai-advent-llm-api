...
Проект последовательно знакомит с работой через LLM API: от первого запроса и сохранения истории диалога до управления форматом, сравнения способов рассуждения, исследования температуры, сопоставления моделей и создания отдельного агента с постоянной памятью.
...
Программы первого, второго и шестого дня работают в интерактивном режиме. Для завершения введите:

```text
выход
```

Шестой день также поддерживает команды `/exit` для завершения и `/history` для просмотра журнала экспедиции.
...
В шестом задании используется `max_completion_tokens=1200`, а модели передаются только последние 20 завершённых сообщений. Полная история миссии остаётся в SQLite. Неуспешные запросы сохраняются для диагностики, но исключаются из контекста LLM.
...
Первые пять дней исследуют отдельные механизмы. С шестого дня они начинают объединяться в переиспользуемого агента, которого в дальнейшем можно дополнить многоуровневой памятью, тематическими ветками, векторным поиском, саморефлексией, Judge, несколькими провайдерами моделей и REST-интерфейсом.
