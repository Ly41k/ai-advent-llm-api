...
Практический проект по работе с LLM API. Каждый день добавляет один новый механизм: управление ответом, сравнение моделей, постоянную историю, подсчёт токенов, сжатие и стратегии контекста, явные слои памяти, персонализацию, формальное состояние задачи, неизменяемые инварианты и контролируемый жизненный цикл.
...
CLI
  → BublikAgent
    ...
    → lifecycle-проверка относительно этапа задачи
...
- доступность переходов с учётом guards и явное утверждение плана;
- блокировку пропуска этапов и преждевременной реализации;
- сохранение pause/resume в planning, execution, validation и после перезапуска.

## Возможности актуального агента

В Дне 15 Бублик умеет:
...
- показывать только переходы, доступные после проверки текущих guards;
- требовать явного утверждения плана перед execution;
- проверять готовый ответ относительно текущего этапа и expected action;
- блокировать реализацию во время planning и финал без успешной validation.
...
| `/task approve` | Явно утвердить текущий план перед execution |
| `/task status` | Показать этап, expected action, прогресс и доступные сейчас переходы |
...
В День 15 жизненный цикл задачи становится явным и наблюдаемым контрактом. Структурный граф остаётся `planning → execution → validation → done`, а контекстные guards определяют, какой переход действительно доступен в текущий момент.

Перед началом execution план нужно создать и явно утвердить командой `/task approve`. Любое изменение плана сбрасывает утверждение. Переход в validation становится доступен только после завершения всех execution-шагов, а `done` требует сохранённого успешного результата проверки.

Команда `/task status` показывает текущий этап, утверждение плана, прогресс, вычисляемое expected action и только те переходы, guards которых выполнены. Пока задача на паузе, переходы и изменение прогресса недоступны; после resume точный этап и текущий шаг восстанавливаются из SQLite, в том числе после перезапуска.

Semantic postflight также получает активный `TaskContext`. Если модель выдаёт реализацию во время planning, объявляет validation во время execution или формирует финальный результат без успешной проверки, такой ответ отбрасывается и до сохранения заменяется детерминированным отказом.
...
Запуск всех 35 локальных тестов:
...
- В Дне 15 lifecycle-проверка ответа выполняется тем же semantic postflight и работает в fail-closed режиме, если результат guard невозможно проверить.
